...
from SRmodels.GAN import HSIGenerator, HSIPatchDiscriminator3D
...
def get_dataloaders(patch_dir, batch_size=8, test_ratio=0.1, ratio=4):
    ...
    class PairDataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            ...
            # 生成LR: 先下采样再上采样
            lr = F.interpolate(hr, scale_factor=1/ratio, mode='bilinear', align_corners=False, recompute_scale_factor=True)
...
def train_gan(generator, discriminator, train_loader, g_optimizer, d_optimizer, loss_fn, device, adv_weight=1e-3):
    ...
    total_g_loss = 0
    total_d_loss = 0
    from SRmodels.FCNN3D import center_crop
